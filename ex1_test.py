...
def test_output_is_equal_to_name():
    assert name_point.hi_my_name_is() == "VINAYAK"
...
def test_output_starts_with_specific_letter():
    assert name_point.hi_my_name_is().startswith("V")

def test_output_ends_with_specific_letter():
    assert name_point.hi_my_name_is().endswith("K")
